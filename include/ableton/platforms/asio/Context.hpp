...
// This file shadows link/include/ableton/platforms/asio/Context.hpp so that our project
// can extend the Context class with openBoundUdpSocket(), needed by UdpTunnel.
// Keep this file in sync with upstream when updating the link submodule.
...
public:
  ...
  // Opens a UDP socket bound to all interfaces on a specific port.
  // Used by UdpTunnel to listen for incoming tunnel connections on a fixed, known port.
  template <std::size_t BufferSize>
  Socket<BufferSize> openBoundUdpSocket(uint16_t port)
  {
    Socket<BufferSize> socket{*mpService, ::LINK_ASIO_NAMESPACE::ip::udp::v4()};
    socket.mpImpl->mSocket.set_option(
      ::LINK_ASIO_NAMESPACE::ip::udp::socket::reuse_address(true));
    socket.mpImpl->mSocket.bind(
      ::LINK_ASIO_NAMESPACE::ip::udp::endpoint{
        ::LINK_ASIO_NAMESPACE::ip::address_v4::any(), port});
    return socket;
  }
